...
export class WebCryptoService {
  ...
  private getOrCreateSalt(): Uint8Array {
    const saved = localStorage.getItem('safeclick_vault_salt');
    if (saved) {
      return new Uint8Array(this.base64ToBuffer(saved));
    }
    const salt = window.crypto.getRandomValues(new Uint8Array(16));
    localStorage.setItem('safeclick_vault_salt', this.bufferToBase64(salt.buffer));
    return salt;
  }
  ...
  hasVault(): boolean {
    return !!localStorage.getItem('safeclick_vault_salt');
  }

  getNotes(): VaultNote[] {
    const saved = localStorage.getItem('safeclick_vault_notes');
    return saved ? JSON.parse(saved) : [];
  }

  saveNotes(notes: VaultNote[]): void {
    localStorage.setItem('safeclick_vault_notes', JSON.stringify(notes));
  }
}
