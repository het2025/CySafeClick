...
@Component({
  ...
  styles: [`
    ...
    h1 { color: var(--safeclick-navy); margin-bottom: 30px; border-bottom: 2px solid var(--border); padding-bottom: 10px; }
    ...
    .status-card.granted { border-color: var(--safeclick-green); background: rgba(19, 136, 8, 0.05); }
    .status-card.denied { border-color: var(--danger); background: rgba(239, 68, 68, 0.05); }
    
    .primary-btn.saffron { background: var(--safeclick-saffron); color: white; padding: 12px 24px; font-size: 1.1rem; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; margin-top: 20px; }
    .secondary-btn { background: var(--surface); border: 2px solid var(--safeclick-navy); color: var(--safeclick-navy); padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: bold; }
    ...
    input:checked + .slider { background-color: var(--safeclick-green); }
    input:checked + .slider:before { transform: translateX(22px); }
    .slider.round { border-radius: 34px; }
    .slider.round:before { border-radius: 50%; }
    .slider.disabled { background-color: var(--safeclick-green); opacity: 0.5; cursor: not-allowed; }
    ...
    .badge.low { background: var(--safeclick-green); color: white; }
...
export class NotificationSettingsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    try {
      const h = localStorage.getItem('safeclick_alert_history');
  ...
  sendTestNotification() {
    if ('Notification' in window && Notification.permission === 'granted') {
      const notification = new Notification('✅ SafeClick Alerts Active', {
        body: 'This is a test notification. You will now receive alerts for new scams.',
        icon: '/assets/icons/safeclick-shield-192.png',
        badge: '/assets/icons/safeclick-badge-72.png',
