...
@Component({
  selector: 'CySafeClick-pattern-detail',
  standalone: true,
  ...
  styles: [`
    ...
    .section h3 { font-size: 1.25rem; font-weight: 700; border-bottom: 2px solid var(--CySafeClick-saffron); padding-bottom: 0.5rem; margin-bottom: 1rem; display: inline-block; }
    ...
    .step-num { font-size: 0.85rem; font-weight: 700; color: var(--CySafeClick-saffron); text-transform: uppercase; }
    .step p { margin: 0.25rem 0 0 0; line-height: 1.5; }
    
    .real-script .script-card {
      background: #1e293b; color: #f8fafc;
      padding: 1.5rem; border-left: 6px solid var(--CySafeClick-saffron);
    ...
    .check-item input { width: 18px; height: 18px; margin-top: 3px; accent-color: var(--CySafeClick-saffron); }
...
export class PatternDetailComponent {
  ...
  shareLink = computed(() => {
    const text = `⚠️ CySafeClick Alert: ${this.title()}.\n${this.summary()}\nApne parivaar ko batayein. CySafeClick.in/digest`;
