...
@Component({
  selector: 'CySafeClick-ticker-detail-drawer',
  standalone: true,
  ...
  styles: [`
    ...
    .drawer-container {
      ...
      border-top: 2px solid var(--CySafeClick-saffron, #FF9933);
      padding: 24px;
    ...
    .action-section {
      ...
      border-left: 4px solid var(--CySafeClick-saffron, #FF9933);
      padding: 16px;
      margin-bottom: 24px;
      
      h4 { margin: 0 0 8px 0; color: var(--CySafeClick-saffron, #FF9933); }
    ...
    .button-group {
      ...
      .btn-primary { background: var(--CySafeClick-saffron, #FF9933); color: #000; }
...
export class TickerDetailDrawerComponent {
  ...
  shareWarning() {
    if (!this.item) return;
    const text = `🚨 CySafeClick SCAM ALERT 🚨\n\n${this.item.text}\n\n${this.item.textHindi}\n\nCheck live alerts on CySafeClick App.`;
