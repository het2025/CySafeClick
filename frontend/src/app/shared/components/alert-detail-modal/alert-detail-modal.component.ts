...
@Component({
  ...
  styles: [`
    ...
    .severity-badge.low { background: var(--safeclick-green); color: white; }
    ...
    .title { font-size: 1.5rem; margin-bottom: 5px; color: var(--safeclick-navy); line-height: 1.3; }
    ...
    .pill.fill { background: rgba(10, 22, 40, 0.1); color: var(--safeclick-navy); font-weight: bold; }
    [data-theme="dark"] .pill.fill { background: rgba(255,255,255,0.1); color: white; }

    .action-box { background: rgba(255, 153, 51, 0.1); border-left: 4px solid var(--safeclick-saffron); padding: 20px; border-radius: 0 8px 8px 0; margin-bottom: 30px; }
...
export class AlertDetailModalComponent {
  ...
  share() {
    const text = encodeURIComponent(`⚠️ SafeClick Alert: ${this.alert.title}\n\n${this.alert.summary}\n\nStay safe! Check SafeClick App for details.`);
