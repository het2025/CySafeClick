...
- **India Threat Map:** Live interactive heatmaps of cybercrime hotspots across different Indian states (streamed via real-time WebSockets), featuring state-wise filtering, district-level granularity, and historical trend analysis to break down the most common scams geographically.
...
- **Password Lab:** An educational tool featuring real-time, client-side (offline) entropy calculation using the industry-standard zxcvbn algorithm (including predictable keyboard walk detection)—and secure HaveIBeenPwned database cross-referencing via k-Anonymity—alongside biometric passkey (WebAuthn) setup tutorials to help users understand what makes a password truly "unhackable" without sending data to a server.
