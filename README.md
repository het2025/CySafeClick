...
- **Cyber Mitra AI:** A 24/7 bilingual assistant powered by Groq with sub-second response times, leveraging **Retrieval-Augmented Generation (RAG)**, memory-based conversational context spanning across multiple active sessions, voice-to-text input support for regional dialects, and voice-based regional slang understanding to provide accurate, context-aware legal guidance. It acts as a digital first-responder to help users identify if they are being scammed, guides them through recovery steps, and provides specialized support for detecting and reporting loan app harassment tactics (including detecting predatory micro-loan app terms of service).
